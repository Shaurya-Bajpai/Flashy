        ...
        scope.launch {
            ...
            when (category) {
                ...
                Notification.CATEGORY_MESSAGE -> {
                    ...
                    val appRule  = appRules.find { it.packageName == packageName }
                    val resolvedAppName = appRule?.appName?.ifBlank { resolveAppName(packageName) } ?: resolveAppName(packageName)

                    if (appRule != null) {
                        if (appRule.filterMode == "selected") {
                            if (!isAllowed(senderName, appRule.contacts)) {
                                Log.d("FlashNotif", "[$packageName] \"$senderName\" not in per-app whitelist — skipped")
                                return@launch
                            }
                        }
                        ...
                        if (filterMode == "selected") {
                            ...
                            if (!isAllowed(senderName, contacts)) {
                                Log.d("FlashNotif", "[$packageName] \"$senderName\" not in global Apps whitelist — skipped")
                                return@launch
                            }
                        }
                        triggerFlash("NOTIF", senderName = senderName, appPackage = packageName, appName = resolvedAppName)
                    }
                }
            }
        }
